...
class ContextBankTests(unittest.TestCase):
    ...
    def test_focus_is_applied_to_map_and_final_generation_prompts(self):
        ...
        self.assertIn("Treat session evidence as historical", mapped)
        self.assertIn("at most 15 Markdown lines", final)
        self.assertIn("at most 150 words", final)
    ...
    def test_interactive_focus_offers_opinionated_presets(self):
        args = SimpleNamespace(general=False, focus=None, focus_preset=None)
        with mock.patch.object(recall.sys.stdin, "isatty", return_value=True), \
                mock.patch("builtins.input", side_effect=["", "durable"]):
            self.assertEqual(recall._creation_focus(args), ("Durable project focus", "durable"))

        with mock.patch.object(recall.sys.stdin, "isatty", return_value=True), \
                mock.patch("builtins.input", return_value="history"):
            self.assertEqual(recall._creation_focus(args), ("Decision history focus", "decision-history"))

    def test_custom_focus_uses_conservative_custom_policy(self):
        args = SimpleNamespace(general=False, focus="  Query\narchitecture  ", focus_preset=None)
        focus, preset = recall._creation_focus(args)
        prompt = recall._final_generation_prompt(["proposal"], "streambed", focus, preset)

        self.assertEqual((focus, preset), ("Query architecture", "custom"))
        self.assertIn("preserve whether claims are implemented, proposed, superseded, or uncertain", prompt)

    def test_custom_focus_is_single_line_and_bounded(self):
        args = SimpleNamespace(general=False, focus="x" * 241, focus_preset=None)
        with self.assertRaisesRegex(recall.ContextError, "exceeds 240 characters"):
            recall._creation_focus(args)

    def test_oversized_initial_draft_is_compressed_once(self):
        oversized = "# project\n\n" + "\n".join(f"- Detail {i}" for i in range(20))
        compact = "# project\n\n## Decision\n\n- Use Markdown.\n"

        with mock.patch.object(recall, "_run_pi_generation", side_effect=[oversized, compact]) as generate:
            draft = recall._generate_creation_draft(
                "project", "Durable project focus", "durable", ["evidence"], None,
            )

        self.assertEqual(draft, compact)
    ...
    def test_source_prompt_bounds_listing_without_truncating_selected_evidence(self):
        snapshot = {
            "files": [{"path": "README.md", "content": "END-OF-EVIDENCE", "bytes": 15, "truncated": False}],
            "listing": [f"path/{i}/" + "x" * 2000 for i in range(100)],
            "skipped": [], "bytesRead": 15,
        }

        prompt = recall.source_generation_prompt("project", None, snapshot, "durable")

        self.assertLessEqual(len(prompt), SOURCE_MAX_EVIDENCE_CHARS)
        self.assertIn("END-OF-EVIDENCE", prompt)
        self.assertTrue(prompt.rstrip().endswith("END-OF-EVIDENCE"))
    ...
    def test_generate_context_with_pi_and_deterministic_source(self):
        ...
        self.assertIn("> Historical evidence through `1970-01-01T00:00:02+00:00`: `abcd-1234`", text)
        self.assertIn("Use Markdown", text)
        self.assertIn("> Historical evidence through `1970-01-01T00:00:02+00:00`: `abcd-1234`", text)
        generate.assert_called_once()
        self.assertEqual(generate.call_args.args[1], "test/model")

    def test_initial_context_including_provenance_is_at_most_twenty_lines(self):
        conn = self._generation_db()
        args = SimpleNamespace(
            name="events-db", session=["abcd"], source=None, focus=None, focus_preset="durable",
            general=False, model=None, dry_run=False, yes=True, force=False, draft_only=False, json=False,
        )
        body = "# Events DB\n\n## Theme\n\n" + "\n".join(f"- Point {i}" for i in range(10))
        with mock.patch.object(recall, "_session_title", return_value="x" * 500), \
                mock.patch.object(recall, "_run_pi_generation", return_value=body):
            path = recall._context_create_evidence(conn, args)

        text = path.read_text(encoding="utf-8")
        self.assertLessEqual(len(text.strip().splitlines()), 20)
